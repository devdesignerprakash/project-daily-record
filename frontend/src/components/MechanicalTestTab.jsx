...
import { PlusCircle, Calendar, Wrench, Pencil } from "lucide-react";
...
export default function MechanicalTestTab({ records, onSuccess, onError }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.count) {
      onError("यान्त्रिक परीक्षण संख्या अनिवार्य छ।");
      return;
    }
    try {
      if (editingId) {
        await api.put(`/api/mechanical-test/${editingId}`, form);
        onSuccess("यान्त्रिक परीक्षण डाटा सफलतापूर्वक सम्पादन भयो!");
      } else {
        await api.post("/api/mechanical-test", form);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <Card className="lg:col-span-1 border border-slate-200 dark:border-zinc-800 shadow-sm">
        <CardHeader>
          <CardTitle className="text-base font-bold flex items-center gap-2">
            {editingId ? <Pencil className="w-4 h-4 text-amber-600" /> : <PlusCircle className="w-4 h-4 text-orange-600" />}
            {editingId ? "प्रविष्टि सम्पादन (Edit Entry)" : "नयाँ प्रविष्टि (New Entry)"}
          </CardTitle>
          <CardDescription className="text-xs">
            {editingId ? "छानिएको रेकर्ड सम्पादन गरी पुन: सबमिट गर्नुहोस्" : "दैनिक यान्त्रिक परीक्षण संख्या दर्ता गर्नुहोस्"}
          </CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="gap-2">
            ...
            <Button
              type="submit"
              className={`flex-1 text-white rounded-md text-sm ${editingId ? "bg-amber-600 hover:bg-amber-500" : "bg-orange-700 hover:bg-orange-600"}`}
            >
      ...
      <Card className="lg:col-span-2 border border-slate-200 dark:border-zinc-800 shadow-sm">
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            आज दाखिला भएका यान्त्रिक परीक्षण विवरण
          </CardDescription>
        </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-slate-50 dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800">
              <TableRow>
                ...
                <TableHead className="text-xs">परीक्षण संख्या</TableHead>
              ...
              {records.length === 0 ? (
                ...
                records.map((row) => (
                  ...
                  >
                    ...
                    <TableCell className="font-bold text-orange-900 dark:text-orange-400">
                      {row.count}
                    </TableCell>
